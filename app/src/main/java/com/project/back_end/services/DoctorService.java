...
@Transactional(readOnly = true)
public List<String> getDoctorAvailability(Long doctorId, LocalDateTime start, LocalDateTime end) {
    ...
    List<String> bookedSlots = bookedAppointments.stream()
            .map(appointment -> appointment.getAppointmentTime().format(formatter))
...
public int updateDoctor(Doctor updatedDoctor) {
    return doctorRepository.findById(updatedDoctor.getId()).map(existingDoctor -> {
