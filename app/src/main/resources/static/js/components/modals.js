...
export function openModal(type) {
  ...
  if (type === 'addDoctor') {
    ...
    modalContent = `
        <h2>Doctor Login</h2>
        <input type="text" id="username" placeholder="Email" class="input-field">
        <input type="password" id="password" placeholder="Password" class="input-field">
        <button class="dashboard-btn" id="doctorLoginBtn">Login</button>
      `;
  }

  // HTML in das Modal rendern und anzeigen
  document.getElementById('modal-body').innerHTML = modalContent;
  document.getElementById('modal').style.display = 'block';

  // Schließen-Event Handler
  ...
  if (type === 'adminLogin') {
    if (typeof window.loginAdmin === 'function') {
      document.getElementById('adminLoginBtn').addEventListener('click', window.loginAdmin);
    }
  }

  if (type === 'doctorLogin') {
    if (typeof window.loginDoctor === 'function') {
      document.getElementById('doctorLoginBtn').addEventListener('click', window.loginDoctor);
    }
  }
}
